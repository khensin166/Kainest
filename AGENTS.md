...
12. **Options API wajib mendaftarkan komponen di `components: {}`**: `import` saja tidak cukup. Tanpa registrasi, Vue merender tag literal seperti `<iconmenu>` — elemen HTML tak dikenal berukuran **0×0**. Tidak ada error di console, build hijau, elemennya sekadar hilang. Ini pernah membuat tombol hamburger sidebar tak terlihat di mobile sehingga navigasi mati total. Dijaga aturan lint `no-unregistered-comp`.
13. **Jangan menganimasikan `transform` untuk memusatkan elemen**: Tailwind v4 memakai properti `translate` (bukan `transform`) untuk `-translate-x-1/2`. Keduanya berlaku bersamaan sehingga pergeserannya berlipat. Animasikan `scale`/`opacity` saja.

          ...
          >
            <div class="flex items-center gap-4">
              ...
              <div class="text-left">
                ...
                <p class="text-xs text-gray-400 dark:text-gray-500 mt-0.5">
                  <span class="text-emerald-500 font-medium">Masuk: {{ formatRupiah(item.totalIncome) }}</span> | 
                  <span class="text-red-500 font-medium">Keluar: {{ formatRupiah(item.totalSpent) }}</span>
                </p>
              </div>
            </div>
            ...
            <div class="hidden sm:flex items-center gap-6 text-right">
              ...
              <div>
                ...
                <p class="text-sm font-bold" :class="(item.totalSpent > item.totalIncome && item.totalIncome > 0) ? 'text-red-500' : 'text-red-400'">
                  {{ formatRupiah(item.totalSpent) }}
                </p>
              </div>
              <div>
                <p class="text-[10px] text-gray-400 uppercase tracking-wide">Tabungan</p>
                <p class="text-sm font-bold text-emerald-700 dark:text-emerald-300">{{ formatRupiah(item.totalSaved) }}</p>
              </div>
              <ChevronDownIcon
              ...
              <div class="px-6 py-6 space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
                   ...
                   <div class="md:col-span-4 lg:col-span-3 flex flex-col items-center">
                     <p class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-4 w-full text-center">
                        Alokasi Pengeluaran
                     </p>
                     <div class="relative w-48 h-48 mb-2">
                        <Doughnut v-if="getDonutData(item).datasets[0].data.length > 0" :data="getDonutData(item)" :options="donutOptions" />
                        <div v-else class="absolute inset-0 flex items-center justify-center text-xs text-gray-400 text-center">Belum ada pengeluaran</div>
                     </div>
                   </div>
                   ...
                   <div class="md:col-span-8 lg:col-span-9">
                     ...
                     >
                        <div
                           v-for="(pocket, index) in sortedPockets(item.pocketsSnapshot)"
                           :key="pocket.categoryId"
                           class="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-700/40 rounded-xl"
                        >
                           <div class="text-lg w-8 text-center font-bold text-gray-300 dark:text-gray-600">#{{ index + 1 }}</div>
                ...
                <div v-if="item.aiEvaluation" class="p-4 rounded-xl bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-800/50 mt-4">

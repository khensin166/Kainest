<template>
  <div class="bg-surface-card rounded-2xl border border-border-default shadow-none overflow-hidden">
    ...
    <div class="flex items-center gap-2 px-5 py-4 border-b border-border-default">
      <MegaphoneIcon class="w-4 h-4 text-brand-primary" />
      ...
      <button v-if="isAdmin" @click="syncGithub" :disabled="syncing"
        class="ml-auto text-xs flex items-center gap-1 bg-brand-surface text-brand-primary hover:bg-brand-surface/80 px-2 py-1 rounded-lg transition-colors disabled:opacity-50">
    ...
    <div v-if="loading" class="p-5 space-y-4">
      <div v-for="i in 3" :key="i" class="flex items-start gap-3">
        ...
        <div class="flex-1 space-y-1.5">
          <div class="h-3 bg-surface-hover rounded animate-pulse w-1/4"></div>
    ...
    <div v-else-if="updates.length === 0" class="py-8 text-center">
      <p class="text-sm text-text-muted">Belum ada update sistem.</p>
    </div>

    <!-- Updates List -->
    <ul v-else class="divide-y divide-border-default max-h-[400px] overflow-y-auto">
      <li v-for="update in updates" :key="update.id"
        class="flex items-start gap-3 px-5 py-4 hover:bg-surface-hover transition-colors">
        <div class="mt-0.5 w-8 h-8 rounded-xl bg-brand-surface flex items-center justify-center flex-shrink-0">
          <RocketLaunchIcon class="w-4 h-4 text-brand-primary" />
        </div>
        <div class="flex-1 min-w-0">
          <div class="flex items-center gap-2 mb-0.5 flex-wrap">
            <span class="text-xs font-bold text-brand-primary bg-brand-surface px-2 py-0.5 rounded-full">{{ update.version }}</span>
            <span class="text-xs text-text-muted">{{ formatDate(update.date) }}</span>
          </div>
          <a v-if="update.url" :href="update.url" target="_blank" class="text-sm font-medium text-text-primary hover:text-brand-primary hover:underline inline-block">{{ update.title }}</a>
        ...
        <span v-if="update.badge" class="text-xs font-medium px-2 py-0.5 rounded-full flex-shrink-0 bg-status-success/20 text-status-success">
          {{ update.badge }}
        </span>
      </li>
    </ul>
